...
class MICRON(nn.Module):
    ...
    def forward(self, input):
        ...
        health_representation = torch.cat([diag_emb, prod_emb], dim=-1).squeeze(dim=0)
        health_representation_last = torch.cat([diag_emb_last, prod_emb_last], dim=-1).squeeze(dim=0)

        health_rep = self.health_net(health_representation)[-1:, :]
        health_rep_last = self.health_net(health_representation_last)[-1:, :]
